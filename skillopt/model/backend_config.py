...
def set_student_backend(backend: str) -> None:
    ...
    if STUDENT_BACKEND not in {"openai_chat", "claude_chat", "codex_exec", "claude_code_exec"}:
        raise ValueError(
            f"Unsupported student backend: {STUDENT_BACKEND!r}. "
            "Supported values are 'openai_chat', 'claude_chat', 'codex_exec', and 'claude_code_exec'."
        )
...
def is_student_chat_backend() -> bool:
    return STUDENT_BACKEND in {"openai_chat", "claude_chat"}
